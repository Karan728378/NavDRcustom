...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        export = Button(this).apply { text = "Export latest stopped session"; setOnClickListener { chooseExport() }; layout.addView(this) }
    ...
    private fun render() {
        ...
        export.isEnabled = !s.recording && !s.finalizing && !exporting
    ...
    private fun chooseExport() {
        exportFile = File(filesDir, "recordings").listFiles()?.filter { it.extension in listOf("jsonl", "partial") }?.maxByOrNull { it.lastModified() }
        val file = exportFile
        if (file == null) { Toast.makeText(this, "No recordings yet", Toast.LENGTH_SHORT).show(); return }
